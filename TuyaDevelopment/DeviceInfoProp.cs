...
namespace TuyaDevelopment
{
    ...
    {
        public const string device_id_sensor = "your device id"; // door sensor
        ...
        private static bool battery_flag = false;
        ...
        public static bool CheckLastState(string deviceID)
        {
            ...
            switch (deviceID)
            {
                case device_id_sensor:
                    var battery_percentage = myDeserializedClass.Result[1].Value;
                    int perctange = Convert.ToInt16(battery_percentage);

                    if (perctange < 50 && battery_flag==false)
                    {
                        m_notifyIcon.BalloonTipText = "Sensor Battery: % "+perctange;
                        m_notifyIcon.BalloonTipTitle = "The Sensor App";
                        m_notifyIcon.Text = "Battery Notification";
